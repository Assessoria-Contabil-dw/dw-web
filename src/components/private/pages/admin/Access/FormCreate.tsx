...
export default function FormCreate({id, closeModel}: FormProps){
  ...
  const { mutate, isLoading } = useMutation({
    ...
    mutationFn: () => adminService.createAccess({
      id,
      partyCode: String(search.partyCode),
      stateId: search.acessType === 'E' ? String(search.stateId): undefined,
      cityCode: search.acessType === 'M' ? String(search.cityCode) : undefined,
      modules: search.modules,
    }),
    onSuccess: () => {
      notify({ type: "success", message: "Acesso criado com sucesso" });
